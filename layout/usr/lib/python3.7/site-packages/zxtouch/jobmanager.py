import json
from dataclasses import dataclass, field
from datetime import datetime
from pathlib import Path
from typing import Any, Dict, Optional

from .plan import Plan


@dataclass
class JobState:
    plan: Optional[Plan] = None
    current_step_id: Optional[str] = None
    step_status: Dict[str, str] = field(default_factory=dict)
    last_seq: Dict[str, Dict[str, int]] = field(default_factory=dict)
    retry_counters: Dict[str, int] = field(default_factory=dict)
    lease_info: Dict[str, Any] = field(default_factory=dict)
    budgets_used: Dict[str, int] = field(default_factory=dict)
    timestamps: Dict[str, str] = field(default_factory=dict)
    job_registry: Dict[str, Dict[str, Any]] = field(default_factory=dict)

    def to_dict(self) -> Dict[str, Any]:
        return {
            "plan": self.plan.to_dict() if self.plan else None,
            "current_step_id": self.current_step_id,
            "step_status": self.step_status,
            "last_seq": self.last_seq,
            "retry_counters": self.retry_counters,
            "lease_info": self.lease_info,
            "budgets_used": self.budgets_used,
            "timestamps": self.timestamps,
            "job_registry": self.job_registry,
        }

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "JobState":
        plan_data = data.get("plan")
        return cls(
            plan=Plan.from_dict(plan_data) if plan_data else None,
            current_step_id=data.get("current_step_id"),
            step_status=dict(data.get("step_status", {})),
            last_seq=dict(data.get("last_seq", {})),
            retry_counters=dict(data.get("retry_counters", {})),
            lease_info=dict(data.get("lease_info", {})),
            budgets_used=dict(data.get("budgets_used", {})),
            timestamps=dict(data.get("timestamps", {})),
            job_registry=dict(data.get("job_registry", {})),
        )


class JobManager:
    def __init__(self, state_path: str) -> None:
        self.state_path = Path(state_path)
        self.state = JobState()
        self.autonomous_mode = False

    def load_state(self) -> JobState:
        if not self.state_path.exists():
            return self.state
        data = json.loads(self.state_path.read_text(encoding="utf-8"))
        self.state = JobState.from_dict(data)
        self.autonomous_mode = self._lease_expired()
        return self.state

    def update_lease(self, lease_info: Dict[str, Any]) -> None:
        self.state.lease_info = lease_info
        self.autonomous_mode = self._lease_expired()
        self.checkpoint()

    def record_step_status(self, step_id: str, status: str) -> None:
        self.state.step_status[step_id] = status
        self.checkpoint()

    def update_last_seq(self, plan_id: str, step_id: str, seq: int) -> None:
        self.state.last_seq.setdefault(plan_id, {})[step_id] = seq
        self.checkpoint()

    def _lease_expired(self) -> bool:
        expiry = self.state.lease_info.get("lease_expiry_ts")
        if not expiry:
            return False
        try:
            return datetime.utcnow() >= datetime.fromisoformat(expiry)
        except ValueError:
            return False

    def checkpoint(self) -> None:
        self.state_path.parent.mkdir(parents=True, exist_ok=True)
        self.state_path.write_text(json.dumps(self.state.to_dict(), indent=2), encoding="utf-8")

    def restore_plan(self, plan: Plan) -> None:
        self.state.plan = plan
        if plan.steps:
            self.state.current_step_id = plan.steps[0].step_id
        self.checkpoint()

    def start(self, plan_id: str, plan: Plan, owner: str) -> str:
        job_id = f"job_{int(datetime.utcnow().timestamp())}"
        self.state.plan = plan
        self.state.current_step_id = plan.steps[0].step_id if plan.steps else None
        self.state.timestamps["started_at"] = datetime.utcnow().isoformat()
        self.state.job_registry[job_id] = {
            "job_id": job_id,
            "plan_id": plan_id,
            "owner": owner,
            "state": "RUNNING",
            "current_step_index": 0,
            "last_step": self.state.current_step_id,
            "started_at": self.state.timestamps["started_at"],
        }
        self.checkpoint()
        return job_id

    def stop(self, job_id: str, reason: str, lease_manager: Optional[Any] = None) -> None:
        job = self.state.job_registry.get(job_id)
        if not job:
            return
        job["state"] = reason if reason in {"FINISHED", "FAILED", "HUNG"} else "STOPPED"
        job["stop_reason"] = reason
        job["stopped_at"] = datetime.utcnow().isoformat()
        self.checkpoint()
        if lease_manager:
            lease_manager.invalidate_job(job_id)

    def status(self, job_id: str) -> Optional[Dict[str, Any]]:
        return self.state.job_registry.get(job_id)

    def list_jobs(self) -> Dict[str, Dict[str, Any]]:
        return self.state.job_registry

    def update_checkpoint(self, job_id: str, step_index: int) -> None:
        job = self.state.job_registry.get(job_id)
        if not job:
            return
        job["current_step_index"] = step_index
        job["last_step"] = self.state.current_step_id
        job["updated_at"] = datetime.utcnow().isoformat()
        self.checkpoint()
