...
class DaemonCore:
    ...
    def handle_logs(self, cursor: Optional[str]) -> Dict[str, Any]:
        data = self.telemetry.get_logs(cursor)
        return LogsResponse(plan_id=self.job_manager.state.plan.plan_id if self.job_manager.state.plan else "",
                            entries=data["entries"],
                            next_cursor=data["next_cursor"]).to_dict()
