...
class Watchdog:
    ...
    def check_progress_timeout(self, last_progress_ts: datetime, timeout_ms: int, now: Optional[datetime] = None) -> bool:
        now = now or datetime.utcnow()
        return now - last_progress_ts > timedelta(milliseconds=timeout_ms)
